...
def process_words(audio, words, config, initial_offset=0, timer_sound=None):
    ...
    for word_info in words:
        ...
        if word in trigger_words_lower:
            ...
            if timer_sound:
                # Add the timer sound once, even if it is shorter than the pause duration
                output_audio += timer_sound
                # If the timer sound is shorter than the pause duration, fill the rest with silence
                remaining_duration = config["question_pause_duration"] - len(
                    timer_sound
                )
                if remaining_duration > 0:
                    output_audio += AudioSegment.silent(duration=remaining_duration)
            else:
